...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.3"* ]] || [[ $IMAGETAG == "latest" ]]; then
    ...
    for PEER in peer0.org1.example.com peer0.org2.example.com peer0.org3.example.com peer0.org4.example.com; do
...
function replacePrivateKey() {
  ...
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed -it "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
...
# this crypto implementation.DDD
